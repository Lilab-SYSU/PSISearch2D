...
################################################################
# copyright (c) 2016 by Weizhong Li and his team at 
# Sun Yat-sen University and 
# William R. Pearson and The Rector &
...
use vars qw( $query_file $db_file $num_iter $evalue $int_mask $end_mask $query_mask $no_msa $tmp_dir $dom_flag $align_flag $suffix $srch_pgm $file_out $help $shelp $error_log $rm_flag $annot_type $quiet $domain_ann $MySQLDB $DBname $DBuser $DBpasswd);
...
my $root = dirname($Bin);
my $pgm_bin = "$root/bin";
my $pgm_data = "$root/data";
my $ssearch_bin = "$pgm_bin/ssearch36";
my $psiblast_bin = "$pgm_bin/psiblast";
my $ggsearch_bin = "$pgm_bin/ggsearch36";
my $glsearch_bin = "$pgm_bin/glsearch36";
my $makeblastdb_bin = "$pgm_bin/makeblastdb";
my $datatool_bin = "$pgm_bin/datatool -m $pgm_data/NCBI_all.asn";
my $scripts_dir = "$root/scripts";
my $align2msa_lib = "$scripts_dir/m89_btop_msa2.pl";

my %srch_subs = ('ssearch' => \&get_ssearch_cmd,
		             'psiblast' => \&get_psiblast_cmd,
                 'ggsearch' => \&get_ggsearch_cmd,
                 'glsearch' => \&get_glsearch_cmd,
		);

my %annot_cmds = ('rpd3' => qq("\!$scripts_dir/ann_pfam28.pl --pfacc --db RPD3 --vdoms --split_over"),
		  'rpd3nv' => qq("\!$scripts_dir/ann_pfam28.pl --pfacc --db RPD3 --split_over"),
		  'rpd3nvn' => qq("\!$scripts_dir/ann_pfam28.pl --pfacc --db RPD3 --split_over --neg"), 
		  'uniprot' => qq("\!$scripts_dir/ann_upfeats_pfam_www_e.pl --neg --vdoms --bound_comment"),
		  'pfam' => qq("\!$scripts_dir/ann_pfam_www.pl --neg --vdoms --bound_comment")
...
# unless (exists $ENV{"BLASTDB"}){
#   $ENV{"BLASTDB"} = "/lilab/data/blastdb/";
# }
# unless (exists $ENV{"FASTA3DB"}){
#   $ENV{'FASTA3DB'} = '/lilab/data/fastadb/fasta3db';
# }
...
for (my $it=2; $it <= $num_iter; $it++) {
  ...
  my ($search1,$search2) = $srch_subs{$srch_pgm}($query_file, $db_file, $prev_pssm);
#  log_system("$search > $this_file_out 2> $this_file_out.err");
  # tmp modification  for additional output
  if($srch_pgm =~ m/ssearch|ggsearch|glsearch/){
  my $bb_out_cmd = "-m \"FBB $this_file_out.bb\"";
  log_system("$search1 $bb_out_cmd > $this_file_out 2> $this_file_out.err");
  }else{ log_system("$search1 > $this_file_out.bb 2> $this_file_out.err");
         log_system("$search2 > $this_file_out 2> $this_file_out.err");
}
  ...
  copy("$file_out.it$it.bb",$file_out) or die "Copy failed: $!";
  copy("$file_out.it$it","$file_out.tab") or die "Copy failed: $!";

  ($this_pssm, $this_bound_out) = build_msa_pssm($query_file, $this_file_out, $prev_bound_in);
  copy("$file_out.it$it.msa","$file_out.msa");
...
# annotation domain
if ($domain_ann){
  my $command = annotate($file_out,$domain_ann,$MySQLDB,$DBuser,$DBpasswd);
  log_system("$command");
}
...
sub get_ssearch_cmd {
  ...
  # $db_file =~ s/\s/\+/;
  # $db_file="+".$db_file."+";
  my $search_cmd = qq($ssearch_bin -S -L -m 8CB -d 0 -E "1.0 0" -s BP62);
  if ($annot_type) {
...
# builds an ggsearch command line with query, db, and pssm
#
sub get_ggsearch_cmd {
   my ($query_file, $db_file, $pssm_file) = @_;
   # $db_file =~ s/\s/\+/;
   # $db_file="+".$db_file."+";
   my $search_cmd = qq($ggsearch_bin -S -L -m 8CB -d 0 -E "1.0 0" -s BP62);
   if ($annot_type) {
      $search_cmd .= qq( -V $annot_cmds{$annot_type});
   }
   if ($pssm_file) {
...
sub get_glsearch_cmd {
   ...
   # $db_file =~ s/\s/\+/;
   # $db_file="+".$db_file."+";
   my $search_cmd = qq($glsearch_bin -S -L -m 8CB -d 0 -E "1.0 0" -s BP62);
   if ($annot_type) {
      $search_cmd .= qq( -V $annot_cmds{$annot_type});
   }
   if ($pssm_file) {
...
sub get_psiblast_cmd {
  my ($query_file, $db_file, $pssm_file) = @_;
   $db_file=~ s/\.fasta//;
  # my @db = split(/\s+/, $db_file);
  # $db_file = "";
  # # print "###############################",@db,$ENV{'BLASTDB'};
  # # $db_file = join ($ENV{'BLASTDB'},@db);
  # # print "###############################",$db_file;
  # foreach (@db){
  #   $db_file .= " ".$ENV{'BLASTDB'}.$_;
  # }
  # $db_file = qq("$db_file");
  my $form = qq('7 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore score btop');
  my $search_cmd1 = qq($psiblast_bin -num_threads 4 -max_target_seqs 5000 -outfmt $form -inclusion_ethresh $evalue -num_iterations 1 -db $db_file);
  # print "################",$search_cmd1."\n";
  my $search_cmd2 = qq($psiblast_bin -num_threads 4 -max_target_seqs 5000 -outfmt 0 -inclusion_ethresh $evalue -num_iterations 1 -db $db_file);
  if ($pssm_file) {
    $search_cmd1 .= qq( -in_pssm $pssm_file);
    $search_cmd2 .= qq( -in_pssm $pssm_file);
#    $search_cmd .= qq( -comp_based_stats 0);
  }
  else {
    $search_cmd1 .= qq( -query $query_file);
    $search_cmd2 .= qq( -query $query_file);
  }

  return ($search_cmd2,$search_cmd1);
}
...
sub build_msa_pssm {
  ...
  my $aln2msa_cmd = qq($align2msa_lib --query $query_file --evalue $evalue --masked_lib_out=$this_hit_db --db_num $db_num );
...
#  my $buildpssm_cmd = "$psiblast_bin -max_target_seqs 5000 -outfmt 7 -inclusion_ethresh 100.0 -in_msa $this_msa -db $this_hit_db -out_pssm $this_pssm_asntxt -num_iterations 1 -save_pssm_after_last_round";
  my $buildpssm_cmd = "$psiblast_bin -max_target_seqs 5000 -outfmt 7 -inclusion_ethresh 100.0 -in_msa $this_msa -db $this_hit_db -out_pssm $this_pssm_asntxt -num_iterations 1 -comp_based_stats 0";
...
###################
#sub annotation domain
#
sub annotate{
  my %annot_cmds = ('rpd3' => "$scripts_dir/ann_pfam28.pl --pfacc --db RPD3 --vdoms --split_over",
      'rpd3nv' => "$scripts_dir/ann_pfam28.pl --pfacc --db RPD3 --split_over",
      'rpd3nvn' => "$scripts_dir/ann_pfam28.pl --pfacc --db RPD3 --split_over --neg", 
      'uniprot' => "$scripts_dir/pfam_anno_local.v3.pl uniprot",
      'pfam' => "perl $scripts_dir/pfam_anno_local.v3.pl pfam"
      #'pfam' => qq("\!../scripts/ann_pfam30.pl --pfacc --vdoms --split_over")
    );
  my ($file,$dbtype,$mysqldb,$user,$passwd) = @_;
  $file =~ /([^\.]+)\.?/;
  my $fileout =$1;

  open TEST, "< $file";
  open OUT, "> $fileout.acc";
  while (<TEST>){
 #   if (/^(\w+(\||:)\w+)/){
 #    my @fields = split /\s/,$_;
 #    print OUT $fields[0]."\n";}
   ...
   # if ($dbtype eq "pfam"){
   #  my $command = $annot_cmds{$dbtype}." $fileout.acc";
   # }
   # elsif($dbtype eq "uniprot"){
   #  my $command = $an
   # my $command = "$scripts_dir/ann_pfam_www.pl $fileout.acc >$fileout.domain";
   my $command = $annot_cmds{$dbtype}." $fileout.acc $mysqldb $user $passwd";
   return $command;
}
#
##############################
##cvalue calculate
##modify by Minglei Yang
sub cvalue{
      my ($file,$evaluethreshold) = @_;
      $file =~ /([^\.]+)\.?/;
      my $fileout = $1;
      my $command = "perl $scripts_dir/cvalue.pl $fileout.domain $fileout.result.tab $evaluethreshold";
      return $command;
      }
...
=head1 SYNOPSIS
 ...
 psisearch2d_msa_anno.pl --query q_file --db db_file --pgm ssearch|psiblast|ggsearch|glsearch --num_iter 5
...
=head1 OPTIONS
 ...
 --pgm   program used for searching, ssearch / psiblast / ggsearch / glsearch
 ...
 --domain_ann implement the annotation of domain "uniprot" or "pfam"
 --MySQLDB user's local MySQL database name if domain_ann is needed
 --DBuser MySQL user's username
 --DBpasswd MySQL user's passwd
...
William R. Pearson (wrp@virginia.edu) and Weizhong Li (liweizhong@mail.sysu.edu.cn)
